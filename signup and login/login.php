...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  try {
    ...
    $select = $connection->query("SELECT email, username, id, password FROM $usertable WHERE email = '$email'");
    $result = $select->fetch();

    if (!empty($result)) {
      if (password_verify($password, $result["password"])) {
        echo json_encode(["error" => "verified", "username" => $result["username"], "id" => $result["id"]]);
      } else {
